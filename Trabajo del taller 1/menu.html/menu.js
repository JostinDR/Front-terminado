...
const secciones = {
  ...
  usuarios: `
    ...
    <form id="formUsuarios" style="display:flex; flex-direction:column; gap:10px; max-width:400px; margin:auto;">
...
function gestionarUsuarios() {
  ...
  function renderUsuarios() {
    ...
    lista.innerHTML = usuarios.map((u, i) => `
      <div style="border:1px solid #ccc; border-radius:8px; padding:10px; margin:8px 0; display:flex; justify-content:space-between; align-items:center;">
        <div>
          <strong>${u.nombre}</strong> — <em>${u.rol}</em>
        </div>
        <div>
  ...
  window.editarUsuario = (index) => {
    ...
    const nuevoRol = prompt("Nuevo rol (Usuario / Administrador):", usuarios[index].rol);
